...
import { toast, modal, confirmDialog, promptDialog, fmtRelativeTime } from './ui.js';
...
// ===== IMPERSONATION (read-only Snapshot der User-Perspektive) =====
export async function impersonateUser(userId) {
  if (!userId) throw new Error('userId required');
  let snap;
  try {
    snap = await _rpc('admin_impersonate_snapshot', { p_user_id: userId }, {
      errorMsg: 'Konnte Impersonation-Snapshot nicht laden.',
    });
  } catch { return null; }
  if (!snap) return null;
  const u = snap.user || {};
  const posts = snap.recent_posts || [];
  const listens = snap.recent_listening || [];
  const podcasts = snap.podcasts_owned || [];
  const fmtMs = (ms) => `${Math.round((Number(ms)||0)/60000)} min`;
  const html = `
    <div style="margin:-8px 0 16px;padding:10px 14px;background:rgba(245,158,11,0.1);border-left:3px solid #F59E0B;border-radius:6px;font-size:13px">
      👁 <strong>READ-ONLY-Modus</strong> — du siehst nur was dieser User sieht. Keine Aktionen möglich. (Geloggt im Audit-Log)
    </div>
    <div style="display:flex;gap:14px;align-items:center;margin-bottom:14px">
      ${u.avatar_url ? `<img src="${_escape(u.avatar_url)}" style="width:56px;height:56px;border-radius:50%;object-fit:cover">` : ''}
      <div>
        <div style="font-weight:600;font-size:17px">${_escape(u.full_name || u.username || '–')}</div>
        <div style="color:#888;font-size:13px">@${_escape(u.username || '')} · ${_escape(u.type || 'user')}</div>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px">
      <div style="padding:10px;background:rgba(255,255,255,0.04);border-radius:8px;text-align:center"><div style="font-size:18px;font-weight:700">${snap.followers_count ?? 0}</div><div style="font-size:11px;color:#888">Follower</div></div>
      <div style="padding:10px;background:rgba(255,255,255,0.04);border-radius:8px;text-align:center"><div style="font-size:18px;font-weight:700">${snap.following_count ?? 0}</div><div style="font-size:11px;color:#888">Folgt</div></div>
      <div style="padding:10px;background:rgba(255,255,255,0.04);border-radius:8px;text-align:center"><div style="font-size:18px;font-weight:700">${posts.length}</div><div style="font-size:11px;color:#888">Posts (20)</div></div>
      <div style="padding:10px;background:rgba(255,255,255,0.04);border-radius:8px;text-align:center"><div style="font-size:18px;font-weight:700">${listens.length}</div><div style="font-size:11px;color:#888">Hör-Sessions</div></div>
    </div>
    ${podcasts.length ? `<div style="margin-bottom:14px"><div style="font-size:12px;color:#888;margin-bottom:6px">🎙 Eigene Podcasts (${podcasts.length})</div>${podcasts.map(p => `<div style="padding:8px;display:flex;gap:10px;align-items:center;background:rgba(255,255,255,0.03);border-radius:6px;margin-bottom:4px">${p.cover_image ? `<img src="${_escape(p.cover_image)}" style="width:32px;height:32px;border-radius:6px;object-fit:cover">` : ''}<div style="flex:1"><div style="font-weight:600;font-size:13px">${_escape(p.title || '')}</div><div style="font-size:11px;color:#888">${p.followers_count ?? 0} Follower</div></div></div>`).join('')}</div>` : ''}
    <div style="margin-bottom:14px"><div style="font-size:12px;color:#888;margin-bottom:6px">📝 Letzte Posts</div>${posts.length ? posts.slice(0, 8).map(p => `<div style="padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;margin-bottom:4px;font-size:13px"><div style="color:#bbb;line-height:1.4">${_escape((p.content || '').slice(0, 140))}${(p.content || '').length > 140 ? '…' : ''}</div><div style="font-size:11px;color:#666;margin-top:3px">${_fmtDate(p.created_at)} · ${p.likes || 0} ♥</div></div>`).join('') : '<div style="color:#666;font-size:13px;font-style:italic">Keine Posts</div>'}</div>
    <div><div style="font-size:12px;color:#888;margin-bottom:6px">🎧 Hört zuletzt</div>${listens.length ? listens.slice(0, 8).map(l => `<div style="padding:8px 10px;background:rgba(255,255,255,0.03);border-radius:6px;margin-bottom:4px;font-size:13px"><div style="font-weight:600">${_escape(l.podcast_title || '–')}</div><div style="font-size:12px;color:#999">${_escape(l.episode_title || '')} · ${fmtMs(l.listened_ms)}</div></div>`).join('') : '<div style="color:#666;font-size:13px;font-style:italic">Keine Hör-Daten</div>'}</div>
  `;
  return new Promise((resolve) => {
    modal({
      title: `👁 Impersonation: ${u.username || userId}`,
      bodyHtml: html,
      width: 640,
      buttons: [{ label: 'Schließen', onClick: (close) => { close(); resolve(true); } }],
    });
  });
}
  ...
  const bodyHtml = `
    ...
    <div style="margin-top:16px;padding-top:12px;border-top:1px solid #2a2a2a;">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:8px;" id="pa-user-actions">
        ...
        <button data-act="impersonate" class="btn btn-secondary" style="background:rgba(245,158,11,0.15);color:#F59E0B;border:1px solid rgba(245,158,11,0.4)">👁 Als User ansehen</button>
      </div>
    </div>
    ...
    <div style="margin-top:16px;padding-top:12px;border-top:1px solid #2a2a2a;">
      <div style="font-size:12px;color:#888;margin-bottom:8px;display:flex;align-items:center;gap:6px;">
        <span>📝 Notizen</span>
        <span id="pa-notes-count" style="color:#666;"></span>
      </div>
      <div id="pa-notes-list" style="max-height:220px;overflow-y:auto;margin-bottom:10px;font-size:13px;">
        <div style="color:#666;font-style:italic;">Lade…</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:6px;">
        <textarea id="pa-note-body" rows="2" placeholder="Neue Notiz…" style="width:100%;background:#1a1a1a;color:#eee;border:1px solid #333;border-radius:6px;padding:8px;font-size:13px;font-family:inherit;resize:vertical;"></textarea>
        <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
          <label style="font-size:12px;color:#aaa;display:flex;align-items:center;gap:4px;cursor:pointer;">
            <input type="checkbox" id="pa-note-pinned"> 📌 Anpinnen
          </label>
          <button id="pa-note-save" class="btn btn-primary" style="padding:4px 12px;font-size:13px;">Notiz speichern</button>
        </div>
      </div>
    </div>
  `;

  // --- Notizen-Rendering / Reload-Helper ---
  function _renderNotes(rootEl, notes) {
    const listEl = rootEl.querySelector('#pa-notes-list');
    const countEl = rootEl.querySelector('#pa-notes-count');
    if (!listEl) return;
    if (!Array.isArray(notes) || notes.length === 0) {
      listEl.innerHTML = '<div style="color:#666;font-style:italic;">Noch keine Notizen.</div>';
      if (countEl) countEl.textContent = '';
      return;
    }
    // Pinned zuerst, dann nach created_at desc
    ...
    let _rel;
    try { _rel = fmtRelativeTime; } catch { _rel = null; }
    const relFn = (typeof fmtRelativeTime === 'function') ? fmtRelativeTime : _fmtDate;
    listEl.innerHTML = sorted.map((n) => `
      <div style="background:${n.is_pinned ? 'rgba(245,158,11,0.08)' : '#1a1a1a'};border:1px solid ${n.is_pinned ? 'rgba(245,158,11,0.35)' : '#2a2a2a'};border-radius:6px;padding:8px 10px;margin-bottom:6px;">
        <div style="white-space:pre-wrap;color:#eee;line-height:1.35;">${n.is_pinned ? '📌 ' : ''}${_escape(n.body || '')}</div>
        <div style="margin-top:4px;font-size:11px;color:#888;">
          ${_escape(n.admin_full_name || n.admin_username || 'Admin')} · ${_escape(relFn(n.created_at))}
        </div>
      </div>
    `).join('');
  }

  async function _loadNotes(rootEl) {
    try {
      const notes = await _rpc('admin_user_notes_list', { p_user_id: userId });
      _renderNotes(rootEl, notes || []);
    } catch (e) {
      const listEl = rootEl.querySelector('#pa-notes-list');
      if (listEl) listEl.innerHTML = '<div style="color:#ff4d4f;">Konnte Notizen nicht laden.</div>';
    }
  }
  ...
  return new Promise((resolve) => {
    modal({
      ...
      onMount: (root, close) => {
        ...
        // Notiz speichern
        const saveBtn = root.querySelector('#pa-note-save');
        const bodyEl = root.querySelector('#pa-note-body');
        const pinnedEl = root.querySelector('#pa-note-pinned');
        if (saveBtn && bodyEl) {
          saveBtn.addEventListener('click', async () => {
            ...
            try {
              await _rpc('admin_user_note_add', {
                p_user_id: userId,
                p_body: body,
                p_is_pinned: !!(pinnedEl && pinnedEl.checked),
              }, { successMsg: 'Notiz gespeichert.' });
              bodyEl.value = '';
              if (pinnedEl) pinnedEl.checked = false;
              await _loadNotes(root);
            } catch (e) {
              // Toast schon gezeigt
            } finally {
          ...
          btn.addEventListener('click', async () => {
            ...
            try {
              switch (act) {
                ...
                case 'impersonate': await impersonateUser(userId); return;
              }
