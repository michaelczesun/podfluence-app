...
(function (global) {
  ...
  function injectStyles() {
    ...
    :root {
      ...
      --lx-bg-glass: rgba(255,255,255,0.65);
      --lx-bg-glass-dark: rgba(20,18,30,0.55);
      --lx-border: rgba(255,255,255,0.25);
      --lx-text: #1a1626;
      --lx-text-soft: rgba(26,22,38,0.6);
    ...
    .lx-hero-change.lx-up { background: rgba(34,197,94,0.12); color: rgb(21,128,61); }
    .lx-hero-change.lx-down { background: rgba(239,68,68,0.12); color: rgb(185,28,28); }
    .lx-hero-change.lx-neutral { background: rgba(100,116,139,0.12); color: rgb(71,85,105); }
  ...
  function drawer(opts) {
    ...
    const contentNode = opts.content || null;  // DOM-Element-Variante
    const onClose = typeof opts.onClose === 'function' ? opts.onClose : null;

    const backdrop = el('div', { class: 'lx-backdrop' });
    const root = el('div', { class: 'lx-drawer', style: `width:${width}px;max-width:100vw;` });

    const header = el('div', { class: 'lx-drawer-header' });
    header.appendChild(el('div', { class: 'lx-drawer-title' }, title));
    const closeBtn = el('button', { class: 'lx-drawer-close', 'aria-label': 'Close' }, '&times;');
  ...
  function statHero(opts) {
    ...
    const valNode = el('div', { class: 'lx-hero-value' }, '0');
    ...
    if (opts.icon) {
      const icon = el('div', { class: 'lx-hero-icon' }, opts.icon);
