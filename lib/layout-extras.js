...
(function (global) {
  ...
  function injectStyles() {
    ...
    .lx-hero {
      ...
      background:
        radial-gradient(120% 90% at 0% 0%, rgba(var(--tab-glow,139 92 246), 0.10), transparent 55%),
      ...
      box-shadow:
        ...
        inset 0 0 22px rgba(var(--tab-glow,139 92 246), 0.06),
        0 10px 30px rgba(0,0,0,0.28),
        0 0 0 1px rgba(var(--tab-glow,139 92 246), 0.10);
      color: var(--lx-text);
    ...
    .lx-hero::before {
      ...
      background: linear-gradient(90deg, transparent, rgba(var(--tab-glow,139 92 246), 0.55), transparent);
    }
    .lx-hero-row {
    ...
    .lx-hero-icon {
      ...
      display: flex; align-items: center; justify-content: center;
      background:
        linear-gradient(135deg, rgba(var(--tab-glow,139 92 246),0.22), rgba(var(--tab-glow,139 92 246),0.08));
      color: var(--tab-accent, var(--lx-purple));
      box-shadow: inset 0 0 0 1px rgba(var(--tab-glow,139 92 246),0.18);
      font-size: 26px;
      flex-shrink: 0;
    }
    ...
    .lx-hero-change.lx-up { background: rgba(34,197,94,0.16); color: rgb(74,222,128); box-shadow: inset 0 0 0 1px rgba(34,197,94,0.28); }
    .lx-hero-change.lx-down { background: rgba(239,68,68,0.16); color: rgb(248,113,113); box-shadow: inset 0 0 0 1px rgba(239,68,68,0.28); }
    .lx-hero-change.lx-neutral { background: rgba(148,163,184,0.16); color: rgb(148,163,184); box-shadow: inset 0 0 0 1px rgba(148,163,184,0.24); }
    /* Sparkline-Background (wenn Panel ein .lx-hero-spark Element rein-rendert) */
    .lx-hero-spark { position: absolute; left: 0; right: 0; bottom: 0; height: 38px; opacity: 0.18; pointer-events: none; mix-blend-mode: screen; }
  ...
  function statHero(opts) {
    ...
    const left = el('div');
    left.appendChild(el('div', { class: 'lx-hero-label crm-label-tiny' }, opts.label || ''));
    ...
    const valNode = el('div', { class: 'lx-hero-value crm-mono' }, _vStr);
      ...
      if (iconVal instanceof Node) {
        ...
        if (iconVal.indexOf('<') !== -1) {
          ...
          if (typeof cached === 'function') {
            ...
            icon.innerHTML = '';
            try {
              import('/lib/ui.js?v=20260610q').then(mod => {
