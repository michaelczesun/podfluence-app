...
(function (global) {
  ...
  function statHero(opts) {
    ...
    if (opts.icon) {
      ...
      // Screenshot). Wenn der Wert wie ein Icon-Key aussieht (kurz, kein '<' drin)
      // und ein globales iconHtml(...) existiert, dann auflösen.
      let iconMarkup = opts.icon;
      const looksLikeIconName = (typeof iconMarkup === 'string'
        && iconMarkup.length > 0
        && iconMarkup.length <= 40
        && iconMarkup.indexOf('<') === -1
        && /^[a-z0-9][a-z0-9\-_]*$/i.test(iconMarkup));
      if (looksLikeIconName) {
        const iconHelper = (typeof global !== 'undefined' && global.iconHtml)
          || (typeof window !== 'undefined' && window.iconHtml);
        if (typeof iconHelper === 'function') {
          try { iconMarkup = iconHelper(opts.icon) || opts.icon; } catch (_e) {}
        }
      }
      const icon = el('div', { class: 'lx-hero-icon' }, iconMarkup);
