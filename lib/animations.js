...
// ----------------------------------------------------------------------------
export function skeletonLoader(arg1, arg2) {
  // Backward-kompatibel: akzeptiert (width, height) ODER ein Options-Objekt
  // ({rows|count|lines, height, width, radius}). ~20 Panels riefen es mit
  // einem OBJEKT in String-Kontext auf (innerHTML/Template) -> zeigte vorher
  // "[object HTMLDivElement]". Andere riefen positional + appendChild (Element).
  // Loesung: HTMLElement zurueck (appendChild OK) MIT toString()=outerHTML
  // (innerHTML/Template OK). Damit funktionieren beide Aufruf-Stile.
  const norm = (v, fb) => v == null ? fb : (typeof v === "number" ? `${v}px` : String(v));
  let count = 1, width = "100%", height = "64px", radius = "12px";
  if (arg1 && typeof arg1 === "object") {
    count = Number(arg1.rows ?? arg1.count ?? arg1.lines ?? 1) || 1;
    width = norm(arg1.width, width);
    height = norm(arg1.height, height);
    radius = norm(arg1.radius, radius);
  } else {
    width = norm(arg1, width);
    height = norm(arg2, height);
  }
  const make = () => {
  ...
  if (count > 1) {
    ...
    result.className = "pf-skeleton-stack";
    for (let i = 0; i < count; i++) result.appendChild(make());
  } else {
    result = make();
  }
  // toString-Override: in String-Kontexten (innerHTML =, `${...}`) wird das
  // Element zu seinem outerHTML statt "[object HTMLDivElement]".
  try { result.toString = () => result.outerHTML; } catch (_) {}
  return result;
}
