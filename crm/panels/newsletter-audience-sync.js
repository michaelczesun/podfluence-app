  ...
  const html = `
    <form id="nlSendForm" class="nl-send-form">
      ...
      <div class="form-row">
        ...
        <textarea name="html" rows="10" required placeholder="HTML-Code mit Variablen wie {{name}} oder {{email}}"></textarea>
...
function renderError(container, msg, onRetry) {
  container.innerHTML = `
    <div class="error-state glass-card" style="text-align:center;padding:48px 24px">
      ...
      <p class="muted">Fehler: ${htmlEscape(msg || 'Unbekannter Fehler')}</p>
    ...
    try {
      ...
      const render = ({ status, counts, broadcasts, timeline }) => {
        ...
        if (timeline && timeline.length > 0) {
          ...
          timelineHost.innerHTML = `<div class="chart-empty muted" style="padding:40px;text-align:center">Daten kommen sobald die RPC <code>newsletter_send_timeline</code> Versand-Events der letzten 30 Tage liefert.</div>`
        }
      ...
      try {
        container.innerHTML = `
          <div class="mount-error" style="padding:48px 24px;text-align:center">
            <div style="font-size:48px;margin-bottom:12px">⚠️</div>
            <h3>Panel konnte nicht geladen werden</h3>
            <p class="muted">Fehler: ${(e && (e.message || String(e))) || 'Unbekannter Mount-Fehler'}</p>
            <button class="btn btn-primary" id="mountRetry" style="margin-top:12px">Erneut versuchen</button>
          </div>
        `
        container.querySelector('#mountRetry')?.addEventListener('click', () => {
