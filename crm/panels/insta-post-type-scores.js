...
function aggregate(posts) {
  ...
  const rows = [...byType.values()].map(r => ({
    ...r,
    avgScore: +(r.scoreSum / Math.max(1, r.count)).toFixed(2),
    ...
    try {
      ...
      function renderMain(rows, posts) {
        ...
        try {
          ...
          statHero(h3, { label: 'Score gesamt (Ø)', value: 0, icon: 'activity', accent: '#10B981' })
        ...
        if (v1) { try { countUp(v1, totalPosts) } catch { v1.textContent = fmtNumber(totalPosts) } }
        if (v3) { try { countUp(v3, avgScoreAll, { decimals: 2 }) } catch { v3.textContent = fmtNumber(avgScoreAll) } }
        if (v4) { try { countUp(v4, totalReach) } catch { v4.textContent = fmtNumber(totalReach) } }
        ...
        try {
          ...
          body.querySelector('#bar-chart').innerHTML = `<div class="empty-state mini"><p>Chart konnte nicht gerendert werden.</p></div>`
        }
      ...
      function openTypeDrawer(type) {
        ...
        const content = `
          <div class="drawer-head">
            ...
            <div class="hint">${fmtNumber(row.count)} Posts &middot; Score (Ø) ${fmtNumber(row.avgScore)}</div>
          </div>
